...
BASE_TOOLS="locales binutils file tree sudo bash-completion u-boot-menu u-boot-fdtdir-dtb initramfs-tools openssh-server network-manager dnsmasq-base libpam-systemd ppp wireless-regdb wpasupplicant libengine-pkcs11-openssl iptables systemd-timesyncd vim usbutils dosfstools parted exfatprogs systemd-sysv pkexec arch-install-scripts bluez cloud-guest-utils cloud-init"
